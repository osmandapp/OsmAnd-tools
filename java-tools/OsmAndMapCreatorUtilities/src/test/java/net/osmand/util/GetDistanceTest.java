...
public class GetDistanceTest {
	...
	public void testDistance() throws Exception {
		...
		if (true) {
			...
			dist(2_234_059, new LatLon(4.21838140, 73.47275904),
					new LatLon(-4.60584164, 55.41123560), "Male-Victoria (via-equator-xy)");
		}

		// longer than 20k km (and their parts)
		if (true) {
			dist(9_191_376, new LatLon(0, -154.26),
					new LatLon(40.76570188, -74.00214505), "Center-NewYork (test)");
			dist(6_814_352, new LatLon(-33.70408492, 151.04180026),
					new LatLon(0, -154.26), "Sydney-Center (test-1)");
			dist(6_815_056, new LatLon(-33.70408492, 151.04180026),
					new LatLon(0.01, -154.26), "Sydney-Center (test-2)");
			dist(6_813_648, new LatLon(-33.70408492, 151.04180026),
					new LatLon(-0.01, -154.26), "Sydney-Center (test-3)");
	...
	private void dist(int ideal, LatLon A, LatLon B, String info) throws Exception {
		...
		float deviation = (float) squareRootDist31Meters / (float) ideal * 100 - 100;

		float speedup = squareRootDist31.getCounter() / measuredDist31.getCounter(); // fast method vs slow method

		System.err.printf(
				"Got squareRootDist31() deviation %.2f%% from precise %d m [%s] sqrt-speedup=%.2f\n",
				deviation, getDistanceMeters, info, speedup
		);
...
//		Assert.assertTrue(Math.abs(deviation) < MAX_DEVIATION); // max 1% for squareRootDist31() method TODO get back later
	}
	...
	private class PerformanceTester<T> {
		...
		private int timeout = TEST_PERFORMANCE_TIMEOUT, counter;
