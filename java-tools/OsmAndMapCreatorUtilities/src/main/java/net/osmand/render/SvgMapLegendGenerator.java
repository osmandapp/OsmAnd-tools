...
public class SvgMapLegendGenerator {
	...
	public static final String LEGEND_MAP_DIR = "/web/main/docs/user/map-legend/";
	...
	public static final String OSMAND_TABLE_MDX = "_osmand-table.mdx";
		...
		try {
			...
			try {
				for (GroupDTO group : configGroups) {
					if (group.type != GroupType.FOLDER) {
						continue;
					}
			...
			for (GroupDTO group : configGroups) {
				if (group.type == GroupType.SPRITE) {
					...
					if (name.equals("group")) {
						...
						if (foldername == null) {
							...
							if (foldername != null) {
								...
								tempGroup.type = GroupType.SPRITE;
							} else {
								tempGroup.type = GroupType.EMPTY;
							}
						} else {
							tempGroup.folderName = foldername;
						...
						if (Algorithms.isEmpty(tempGroup.groupName)) {
	...
	private static class ReactComponentsGenerator {
		public static void generate(ArrayList<GroupDTO> groups) throws Exception {
			...
			for (GroupDTO group : groups) {
				...
				if (group.type != GroupType.FOLDER) {
					String anchor = group.anchor;
					components.put(anchor, group);
					continue;
				}
				content = createGroupContent(group);
				String componentName = group.folderName.replaceAll("[^a-zA-Z]", "");
				componentName = LEGEND_ITEM_PREFIX + Algorithms.capitalizeFirstLetter(componentName);
			...
			createLegendMdxFile(components);
		}

		private static void createLegendMdxFile(Map<String, GroupDTO> components) throws Exception {
			StringBuilder legendTableMdxContent = new StringBuilder("""
			...
			for (Map.Entry<String, GroupDTO> component : components.entrySet()) {
				if (component.getValue().type == GroupType.FOLDER) {
					String componentName = component.getKey();
					legendTableMdxContent
							.append("import %s from '@site/src/components/docs/autogenerated/%s.js';\n"
									.formatted(componentName, componentName));
				}
			}
			legendTableMdxContent.append("\n\n");

			for (Map.Entry<String, GroupDTO> component : components.entrySet()) {
				GroupDTO group = component.getValue();
				String groupTitle = group.headerSign + " " + group.groupName + " {#" + group.anchor + "}\n";
				int height = 70 + (group.icons.size() + 2) / 3 * 110;
				legendTableMdxContent.append(groupTitle);
				if (group.type == GroupType.SPRITE) {
					legendTableMdxContent.append(formSpriteGroup(component, height));
				}
				if (group.type == GroupType.FOLDER) {
					legendTableMdxContent.append("<%s height={%d} />\n".formatted(component.getKey(), height));
				}
				legendTableMdxContent.append("\n");
			}
			String componentMapPath = System.getenv("repo_dir") + LEGEND_MAP_DIR + OSMAND_TABLE_MDX;
			try {
				FileWriter writer = new FileWriter(componentMapPath);
				writer.write(legendTableMdxContent.toString());
				writer.close();
			} catch (Exception e) {
		...
		private static StringBuilder formSpriteGroup(Map.Entry<String, GroupDTO> component, int height) {
			GroupDTO group = component.getValue();
			StringBuilder spriteGroup = new StringBuilder("""
					<LegendItemWithProcessing
					  svgPath='/img/map-legend/osmand-%s'
					  height={%d}
					  svgParts={{
					""".formatted(group.folderName, height));
			for (IconDTO icon : group.icons) {
				spriteGroup.append("    '%s': '%s',\n".formatted(icon.id, icon.name));
			}
			spriteGroup.append("""
					  }}
					/>
		...
		private static @NotNull StringBuilder createGroupContent(GroupDTO group) {
			...
			for (IconDTO icon : group.icons) {
				...
				content.append("        '%s' : {poiName:'%s', poiName2:%s, folderName:'%s/%s'},\n"
						.formatted(icon.name, icon.poiName, poiName2, group.folderName, iconName));
			}
			content.append("""
					    }});
					}
