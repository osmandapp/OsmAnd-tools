...
public class SvgMapLegendGenerator {
    ...
    public static void main(String[] args) throws Exception{
        ...
        try {
            ...
            for (GroupDTO group : configGroups) {
                ...
                String resultFolderPath = resultsFolderPath + group.folderName;
                try {
                ...
                for (IconDTO icon : group.icons) {
                    try {
                        ...
                        SvgGenerator.createSvgFile(group, icon, contentDay, true);
                        String contentNight = SvgGenerator.generate(icon.iconName, icon.shieldNameNight, null, 0.f);
                        SvgGenerator.createSvgFile(group, icon, contentNight, false);
                    } catch (Exception e) {
                        e.printStackTrace();
                ...
                if (next == XmlPullParser.END_TAG) {
                    ...
                    if (name.equals("group")) {
                        ...
                        if (Algorithms.isEmpty(tempGroup.groupName) || Algorithms.isEmpty(tempGroup.folderName)) {
                            throw new Exception("ERROR: parseXmlConfig() - group fields invalid at config.xml line " + parser.getLineNumber());
                        }
    ...
    private static class SvgGenerator {
        ...
        public static void createSvgFile(GroupDTO group, IconDTO icon, String content, boolean isDay) throws Exception {
            String postfix = isDay ? "_day.svg" : "_night.svg";
            String path = resultsFolderPath + group.folderName + "/" + group.folderName + "_" + icon.iconName + postfix;
            try {
    ...
    private static class HtmlGenerator {
        public static void generate(ArrayList<GroupDTO> groups) throws Exception {
            for (GroupDTO group : groups) {
                ...
                String path = folderName + "/autogenerated.txt";
                try {
                    FileWriter writer = new FileWriter(path);
                    writer.write(content);
                    writer.close();
