...
- FR-2-1: REST endpoint `POST /admin/test/csv/count{file_name}` that counts the number of rows in a CSV file by
  streaming it once to determine the total row count `N` without loading everything into memory. **This logic is
  reused  internally for Overpass ingestion as defined in FR-1.**
- FR-2-2: REST endpoint `POST /admin/test/csv/retrieve{file_name, count}` that accepts an uploaded file and 
  `sizeLimit` (see below), then stores a random sample in a new dataset. **When called indirectly by FR-1, the 
  temporary Overpass file is passed to this step.** System MUST use streaming processing to avoid memory issues during 
...
- FR-4: For every created dataset the **server SHALL generate the dataset `name` to show for user interface** from the input
  ...
  `name`, `source`, `created`, `updated`, `status`) in table `datasets`.
