...
public class DeleteOldFilesOperation extends AbstractFileFixOperation implements ExtraParamsOperation {
	...
	public Object run(Params params, OperationContext ctx) {
		...
		boolean checkFreeInactive = extra.freeInactive() == null || extra.freeInactive();
		boolean checkOldTrash = extra.oldTrash() == null || extra.oldTrash();
		List<Integer> userIds = sampleUsers(params, ctx);
		AtomicInteger done = new AtomicInteger();
		forEach(threads, userIds, ctx, userId -> {
			processUser(userId, checkFreeInactive, checkOldTrash, params, stats);
	...
	private void processUser(int userId, boolean checkFreeInactive, boolean checkOldTrash, Params params, Stats stats) {
		...
		boolean found = false;
		if (checkFreeInactive && isFreeAndInactive(userId, user, res)) {
