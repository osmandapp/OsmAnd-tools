...
public class SpeedMetricsTest {
	...
	public static void main(String[] args) {
		...
		for (File f : files) {
			...
			SpeedMetrics m = DownloadOsmGPX.computeSpeedMetrics(gpx); // warm up + result

			long start = System.nanoTime();
			for (int i = 0; i < iterations; i++) {
				DownloadOsmGPX.computeSpeedMetrics(gpx);
			}
			long elapsedNs = System.nanoTime() - start;

			System.out.printf("%-40s pts %6d  avg %6.1f km/h  max %8.1f km/h  %8.3f ms/call%n",
					f.getName(), points, m.avgKmh(), m.maxKmh(), elapsedNs / 1_000_000.0 / iterations);
		}
	}
}
