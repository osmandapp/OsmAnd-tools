...
public class ExceptionAnalyzerMain {
	...
	private static class Variables {
		 ...
		 *
		 * If modifying these scopes, delete your previously saved credentials at ~/.credentials/gmail-java-quickstart
		 */
	...
	public static Credential authorize(String clientSecretJson, Variables vars) throws IOException {
		...
		GoogleClientSecrets clientSecrets = GoogleClientSecrets.load(vars.JSON_FACTORY, new InputStreamReader(in));
		in.close();
	...
	public static Gmail getGmailService(String clientSecretFile, Variables vars) throws IOException {
		...
		return new Gmail.Builder(vars.HTTP_TRANSPORT, vars.JSON_FACTORY, credential)
	 ...
	 * @throws IOException
	 */
	public static List<Message> listMessagesWithLabels(Gmail service, String userId, List<String> labelIds, int limit)
			throws IOException {
	...
	public static void getAttachments(List<Message> messages, String userId, Gmail service, Variables vars)
		...
		for (Message messageRef : messages) {
			...
			if (messageIdsLoaded.contains(messageId)) {
				System.out.println("Attachment already downloaded " + messageId + " "
						+ (System.currentTimeMillis() - lastSaved) + " ms !");
			...
			if (parts != null) {
				...
				for (MessagePart part : parts) {
					if (part.getFilename() != null && part.getFilename().length() > 0) {
						...
						if (exception.exists()) {
							System.out.println("Attachment already downloaded " + msgId + " "
									+ (System.currentTimeMillis() - lastSaved) + " ms !");
							lastSaved = System.currentTimeMillis();
						} else {
							...
							try {
								...
								attachPart = service.users().messages().attachments().get(userId, messageId, attId)
										.execute();
								...
								fileOutFile.close();
								System.out.println(
										"Attachment saved " + (System.currentTimeMillis() - lastSaved) + "ms !");
								lastSaved = System.currentTimeMillis();
